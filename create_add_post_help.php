...
if (!$res=$mysqli->query("INSERT INTO post_help (id_user, name_post, short_text, long_text, img) VALUES ('$_SESSION[id]', '$_POST[name]', '$_POST[short_text]', '$_POST[long_text]', '$img')")) {
    echo "Не удалось вызвать хранимую процедуру: (" . $mysqli->errno . ") " . $mysqli->error;
} else
    echo 'Успешно';
$insert_id=$mysqli->insert_id;
foreach ($_POST['tags'] as $value){
    if (!$res=$mysqli->query("CALL AddTagForPostHelp('$insert_id','$value')")) {
        echo "Не удалось вызвать хранимую процедуру: (" . $mysqli->errno . ") " . $mysqli->error;
    } else
        echo 'Успешно';
